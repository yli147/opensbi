...
#define SBI_MPXY_STMM_MSG_DATA_MAX_SIZE	(0x4000)
			    ...
			    unsigned long *ack_len)
{
	...
	if (RPMI_SMM_SRV_VER == msg_id) {
		rs = sbi_hartindex_to_domain_rs(
			sbi_hartid_to_hartindex(current_hartid()), dom);
		uint32_t version = MM_VERSION_COMPILED;
		if(respbuf) {
			sbi_memcpy((void *)rs->shmem.shmem_addr_lo, &version, sizeof(version));
			msg_len = sizeof(version);
		}
	} else if (RPMI_SMM_SRV_COMPLETE == msg_id) {
		rs = sbi_hartindex_to_domain_rs(
	...
	} else if (RPMI_SMM_SRV_COMMUNICATE == msg_id) {
		rs = sbi_hartindex_to_domain_rs(
