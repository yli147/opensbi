...
struct mm_rpxy_dispatcher mm_dispatcher;

int mm_setup(void *fdt, int nodeoff, const struct fdt_match *match)
{
...
struct rpxy_mm_data {
    ...
    struct mm_rpxy_dispatcher *srv_dispatcher;
};
			  ...
			  const struct fdt_match *match)
{
    ...
    const struct mm_rpxy_dispatcher *dispatcher = data->srv_dispatcher;
...
static const struct fdt_match rpxy_mm_match[] = {
	{ .compatible = "riscv,rpmi-mm", .data = &mm_data }, 
	{},
};
