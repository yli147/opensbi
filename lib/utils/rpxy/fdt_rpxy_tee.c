...
#define TEE_NSCOMM_BUF_BASE 0xF17F0000
				  ...
				  unsigned long *ack_len)
{
	...
	if (RPMI_TEE_SRV_TEE_ATTR == srv_id) {
		...
		memcpy((void *)TEE_NSCOMM_BUF_BASE, tx, tx_len);
		*ack_len = sizeof(struct rpmi_tee_tx);
		sbi_ecall_tee_domain_enter();
	} else if (RPMI_TEE_SRV_TEE_COMPLETE == srv_id) {
		if(rx) {
			memcpy(rx, (void *)TEE_NSCOMM_BUF_BASE, sizeof(struct rpmi_tee_rx));
...
static const struct fdt_match rpxy_tee_match[] = {
	{ .compatible = "riscv,rpmi-mm", .data = &tee_data }, 
	{},
};
